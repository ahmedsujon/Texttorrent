                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header bg-white" style="border-bottom: 1px solid #e2e2e7;">
                            <h4 class="card-title" style="float: left;">Contact Messages</h4>
                        </div>
                        <div class="card-body">
                            <div class="row mb-3">
                                ...
                                <table class="table align-middle table-nowrap table-bordered mb-0">
                                    ...
                                    <tbody>
                                        @if ($messages->count() > 0)
                                            @php
                                                $sl =
                                                    $messages->perPage() * $messages->currentPage() -
                                                    ($messages->perPage() - 1);
                                            @endphp
                                            @foreach ($messages as $message)
                                                <tr>
                                                    <td class="align-middle">{{ $sl++ }}</td>
                                                    <td class="align-middle">{{ $message->first_name }}
                                                        {{ $message->last_name }}</td>
                                                    <td class="align-middle">{{ $message->email }}</td>
                                                    <td class="align-middle">{{ $message->phone }}</td>
                                                    <td class="align-middle">{{ $message->descriptions }}</td>
...
@push('scripts')
    <script>
        window.addEventListener('contact_deleted', event => {
            ...
            Swal.fire(
                "Deleted!",
                "The contact message has been deleted.",
                "success"
            );
        });
    </script>
